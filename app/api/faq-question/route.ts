...
// keep it the same as quote route
export const runtime = 'nodejs'

export async function POST(req: NextRequest) {
  try {
    // 1) Rate limit – identical to quote
    ...
    // 2) Parse JSON – identical content-type handling as quote
    // (If your quote route supports formData, swap this block to match it.)
    const { name, email, question, turnstileToken, pageSource } = await req.json()

    // 3) Turnstile – same as quote
    ...
    // 4) Compose email – same transport/settings as quote
    const subject = `New FAQ Question from ${name}`
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0A5DB5;">New FAQ Question</h2>
